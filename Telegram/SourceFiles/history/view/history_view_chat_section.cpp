...
void ChatWidget::send(Api::SendOptions options) {
	...
	const auto pluginResult = Core::App().plugins().interceptOutgoingText(
		&session(),
		_history,
		message.textWithTags,
		options);
	switch (pluginResult.action) {
	case Plugins::CommandResult::Action::Cancel:
		return;
	case Plugins::CommandResult::Action::Handled:
		_composeControls->cancelReplyMessage();
		_composeControls->clear();
		finishSending();
		return;
	case Plugins::CommandResult::Action::ReplaceText:
		message.textWithTags = TextWithTags{ pluginResult.replacementText };
		break;
	case Plugins::CommandResult::Action::Continue:
		break;
	}
