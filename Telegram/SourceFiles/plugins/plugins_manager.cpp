...
rem %* still expands to the full original command line ignoring shift, so it
rem would wrongly include "send <peerId>" in the message body. Rebuild the text
rem from the remaining positional args instead.
set "ASTRO_TEXT="
:send_collect_text
if "%~1"=="" goto :send_text_done
if defined ASTRO_TEXT (
  set "ASTRO_TEXT=%ASTRO_TEXT% %~1"
) else (
  set "ASTRO_TEXT=%~1"
)
shift
goto :send_collect_text
:send_text_done
call :ensure_online || exit /b 1
set "ASTRO_PEER=%PEER%"
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
