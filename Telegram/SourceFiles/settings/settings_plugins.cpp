...
QString PluginSourceExactMatchText() {
	return PluginUiText(
...
QString PluginSourceBadgeDetailText(
	...
	if (state.sourceVerified) {
		auto result = PluginSourceExactMatchText();
...
void AddPluginSourceBadge(
	...
	container->widthValue() | rpl::on_next([=](int width) {
		badge->resize(std::max(0, width - kPluginCardContentInsetLeft - kPluginCardContentInsetRight), badgeHeight);
	}, badge->lifetime());
	badge->resize(std::max(0, container->width() - kPluginCardContentInsetLeft - kPluginCardContentInsetRight), badgeHeight);
	...
	AddPluginDescriptionText(
		container,
		PluginSourceBadgeDetailText(state));
...
void DestroyLayoutChildrenSynchronously(not_null<Ui::VerticalLayout*> layout) {
	// Use VerticalLayout::clear() so the layout's internal row bookkeeping is
	// updated together with the widgets. Deleting the child QWidgets directly
	// left dangling rows behind, which crashed on the next layout pass and left
	// stale plugin cards painted underneath after enable/disable/remove.
	layout->clear();
}
...
private:
	void rebuild() {
		...
		AddPluginSourceBadge(_content, *state);
		Ui::AddSkip(_content);
...
void Plugins::rebuildList() {
	...
	for (const auto &state : plugins) {
		...
		// Source verification state (verified / unverified / other version) is
		// shown only in the plugin install/details window, not on the list card.
