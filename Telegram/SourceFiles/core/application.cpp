...
, _plugins(std::make_unique<Plugins::Manager>(this))
...
void Application::forEachWindow(
		Fn<void(not_null<Window::Controller*>)> callback) const {
	enumerateWindows(std::move(callback));
}
...
void Application::processCreatedWindow(
	...
	_plugins->notifyWindowCreated(window);
}
...
QString Application::changelogLink() const {
	const auto base = u"https://docs.astrogram.su/changelog"_q;
...
void Application::RegisterUrlScheme() {
	...
	base::Platform::RegisterUrlScheme(base::Platform::UrlSchemeDescriptor{
		...
		.arguments = arguments,
		.protocol = u"tg"_q,
		.protocolName = u"Astrogram Link"_q,
		.shortAppName = u"astrogram"_q,
