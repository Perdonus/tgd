...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Astrogram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start Astrogram!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time Astrogram was not closed properly."_q);
	} else {
		_label.setText(u"Last time Astrogram crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST ASTROGRAM DESKTOP BUILD"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://t.me/astrogramchannel"_q);
	});
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"Astrogram Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.astrogramcrash"_q, u"Astrogram crash report (*.astrogramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::checkingFinished() {
	...
	if (result == "Old") {
		_pleaseSendReport.setText(u"This report is about some old version of Astrogram."_q);
		_sendingState = SendingTooOld;
		updateControls();
		return;
	} else if (result == "Unofficial") {
		_pleaseSendReport.setText(u"You use some custom version of Astrogram."_q);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time Astrogram was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
