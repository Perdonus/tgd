...
TGD_PLUGIN_PREVIEW(
	...
	"1.6",
	"@etopizdesblin",
	"Shows plugin logs in a semi-transparent overlay with filtering, copy and clear actions, and adds the /logs command as a fallback entry point.",
	"",
	"GusTheDuck/24")
...
private:
	void refreshInfo() {
		...
		_info.description = Tr(
			_host,
			"Opens a semi-transparent overlay with client.log, plugins.log and plugins.trace.jsonl, plus quick filters for errors/runtime/recovery and the /logs fallback command.",
			u8"Открывает полупрозрачное overlay-окно с client.log, plugins.log и plugins.trace.jsonl, плюс быстрые фильтры по ошибкам/runtime/recovery и fallback-команду /logs.");
		_info.website.clear();
	}
	...
	Plugins::SettingsPageDescriptor makeSettingsPage() const {
		...
		info.description = Tr(
			_host,
			"Reads client.log, tdata/plugins.log and tdata/plugins.trace.jsonl. Quick buttons switch to error/runtime/recovery slices, and /logs opens the overlay from chat as a fallback.",
			u8"Читает client.log, tdata/plugins.log и tdata/plugins.trace.jsonl. Быстрые кнопки переключают срезы error/runtime/recovery, а /logs открывает overlay прямо из чата как fallback.");
